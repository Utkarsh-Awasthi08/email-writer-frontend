...
function App() {
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      const response = await axios.post("https://email-writer-backend-2.onrender.com/api/email/generate", {
        emailContent,
        tone
      });
    ...
    <Container maxWidth="md" sx={{ py: 4 }}>
      ...
      {generatedReply && (
        <Box sx={{ mt: 3 }}>
          ...
          <Button variant="outlined" sx={{ mt: 2, ml: 2 }} onClick={handleDownload}>
            Download as .txt
          </Button>
        </Box>
      )}
